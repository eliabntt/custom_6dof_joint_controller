...
bool CustomJointController::init(ros::NodeHandle &nh,
	...
	if (!nh_priv.param("joint_names", v, v)) {
		...
		for (int i = 0; i < v.size(); i++) {
			...
			} else if (name.compare("pitch_joint")) {
				p_gain = p_gain_pitch;
				i_gain = i_gain_pitch;
				d_gain = d_gain_pitch;
				i_max = i_max_pitch;
			...
			} else if (name.compare("yaw_joint")) {
				p_gain = p_gain_yaw;
				i_gain = i_gain_yaw;
				d_gain = d_gain_yaw;
				i_max = i_max_yaw;
	...
	// get publishing period
	if (!nh_priv.getParam("odom", current_state)) {
	...
	sub_joint_states = nh.subscribe<nav_msgs::Odometry>(current_state, 1, &CustomJointController::getOdom,
	                                                         this);
...
void CustomJointController::enforceLimits(std::string name, double &position) {
	if (upper_limits_[name] < position) {
		position = upper_limits_[name];
	}
	if (lower_limits_[name] > position) {
		position = lower_limits_[name];
	}
}
...
//		enforceLimits(joint_name, command_position);
		...
		if (joint_name == "roll_joint" || joint_name == "pitch_joint") {
			angles::shortest_angular_distance_with_large_limits(
				current_position,
				command_position,
				lower_limits_[joint_name],
				upper_limits_[joint_name],
				error);
		...
		ROS_INFO_STREAM(joint_name << " commanded " << commanded_velocity);
		joint_pid_controllers_[joint_name].setCurrentCmd(commanded_velocity);
		ROS_INFO_STREAM(joint_name << " commanded " << joint_pid_controllers_[joint_name].getCurrentCmd());
	}
	...
	if (publish_rate_ > 0.0 && last_publish_time_ + ros::Duration(1.0 / publish_rate_) < time) {
		...
		if (joint_state_publisher_->trylock()) {
			...
			for (const auto& joint: joint_pid_controllers_) {
				...
				joint_state_publisher_->msg_.velocity.push_back(joint_pid_controllers_[joint.first].getCurrentCmd());
			}
...
void CustomJointController::getCurrentSetpoint(const trajectory_msgs::MultiDOFJointTrajectory::ConstPtr &msg) {
	...
	local_setpoints["x_joint"].emplace_back(msg->points[0].transforms[0].translation.x);
	local_setpoints["x_joint"].emplace_back(msg->points[0].velocities[0].linear.x);
	local_setpoints["y_joint"].emplace_back(msg->points[0].transforms[0].translation.y);
	local_setpoints["y_joint"].emplace_back(msg->points[0].velocities[0].linear.y);
	local_setpoints["z_joint"].emplace_back(msg->points[0].transforms[0].translation.z);
	local_setpoints["z_joint"].emplace_back(msg->points[0].velocities[0].linear.z);

	// convert ros quaternion to rpy
	tf::Quaternion q(msg->points[0].transforms[0].rotation.x, msg->points[0].transforms[0].rotation.y,
	                 msg->points[0].transforms[0].rotation.z, msg->points[0].transforms[0].rotation.w);
	tf::Matrix3x3 m(q);
	double roll, pitch, yaw;
	m.getRPY(roll, pitch, yaw);
	local_setpoints["roll_joint"].emplace_back(roll);
	local_setpoints["roll_joint"].emplace_back(msg->points[0].velocities[0].angular.x);
	local_setpoints["pitch_joint"].emplace_back(pitch);
	local_setpoints["pitch_joint"].emplace_back(msg->points[0].velocities[0].angular.y);
	local_setpoints["yaw_joint"].emplace_back(yaw);
	local_setpoints["yaw_joint"].emplace_back(msg->points[0].velocities[0].angular.z);
